...
| Agents (Claude Code)            | 5     | PM spec, architect, implementer-tester, QA reviewer, code reviewer (wired into `/run-pipeline`)       |
| Shared Agent Skills             | 18    | Portable Claude/Codex workflows including `/commit`, `/review-pr`, `/plan-migration`, and `/review-design` |
| Legacy Claude-only commands     | 5     | Small workflows not promoted to shared skills                                                         |
...
- [`docs/agentic-tooling.md`](docs/agentic-tooling.md) -- six-file
  ...
  per-tier behavior.
- [`docs/prompt-stats.md`](docs/prompt-stats.md) -- token costs and
  loading graph of every instruction file, per harness (generated by
  `make prompt-stats`).
