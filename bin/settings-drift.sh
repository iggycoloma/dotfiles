...
#
# The sandbox block is intentionally different per tier, and so is the
# leading-token guard hook, which protects sandbox exclusion matching and is
# stripped by the generator where no sandbox runs; everything else
# (permissions, other hooks, statusLine, approval_policy, etc.) MUST stay in
# sync.
...
#
# Class 2 -- Read/Edit parity inside settings.json. Every credential path
# in permissions.deny[] is repeated twice, once per file tool. They are
...
# are declared in PARITY_EDIT_ONLY below; anything else is a bug.
#
# Write(<glob>) is deliberately absent and is itself reported as drift: the file
# permission check never consults those entries and Claude Code warns about each
# one at startup. Edit(<glob>) is what covers every file-editing tool, Write
# included.
...
check_json_drift() {
    local pair="$1" host="$2" container="$3" exclude="$4" label="${5:-$4}"
    ...
    emit_drift "$pair" "$label" "$diff_out"
}
...
check_deny_parity() {
    ...
    if ! findings=$(jq -r --argjson exempt "$exempt_json" '
        ...
        ($edit | strip_exempt)          as $edit_core |
        [
          if ($write | length) > 0 then
            "Write(...) deny entries present -- the permission check ignores them and Claude Code warns about each at startup; fold them into Edit(...): \($write)"
          else empty end,
          if $read != $edit_core then
            "Read differs from Edit (after declared exemptions) -- \(explain($edit_core; $read; "Edit"; "Read"))"
          else empty end
        ] | .[]
    ' "$file" 2>&1); then
...
emit_parity_ok() {
    ...
    if [[ "$JSON_OUTPUT" == true ]]; then
        ...
        log_success "$pair: Read/Edit deny lists in parity"
    fi
}
...
emit_parity_drift() {
    ...
    if [[ "$JSON_OUTPUT" == true ]]; then
        ...
        log_error "$pair: Read/Edit deny lists out of parity"
        printf '%s\n' "$findings" | sed 's/^/    /'
    fi
}
...
main() {
    ...
    check_json_drift \
        ...
        '.sandbox, .env, (.hooks.PreToolUse[]?.hooks[]? | select(.command == "~/.agent-hooks/pre-leading-token-guard.sh"))' \
        ".sandbox, .env, the leading-token guard hook"
