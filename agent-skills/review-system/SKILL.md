...
description: |
  ...
  measured slowdown (use optimize-performance), a readability or structure cleanup with no
  invariant at stake (handle as a direct refactoring request, no skill), or a
  vulnerability hunt (use audit-security).
...
The target is the path, module, service, table, or named capability supplied by the user.
...
- Report performance findings without a measurement, or structure findings with no invariant at stake. The first belongs to `optimize-performance`; the second is a cleanup request, not a review finding.
