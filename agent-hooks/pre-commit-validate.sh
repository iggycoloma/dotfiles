...
# Validate jq is available. Commit-message guardrails should fail closed:
# Codex treats ordinary hook failures as non-blocking, so emit a deny decision.
if ! command -v jq &> /dev/null; then
    printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"jq is required for the commit-message guard"}}'
    exit 0
fi
